		...
		<div class="row">
			...
			<div class="col-md-10">
							...
							<tbody>
								...
								<td>
									<button type="button" class="btn btn-warning btn-xs"
										data-toggle="modal" onclick="haibook(<%=bean.getHid()%>,<%=bean.getCard()%>,'<%=bean.getEndtime()%> 00:00:00',<%=bean.getBid()%>,<%=bean.getAid()%>)">还书</button>
								</td>
							</tbody>
							<%
								}
			    ...
			    function haibook(hid,card,endTime,bid,aid) {
			    	con=confirm("是否还书?"); 
			    	if(con==true){
			    		location.href = "/books/borrowServlet?tip=2&show=1&hid="+hid+"&card="+card+"&endTime="+endTime+"&bid="+bid+"&aid="+aid;
			    	}
			    }
			    </script>
			</div>
